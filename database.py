...
from models import Goal, Task, Scholarship, PhDRadar
...
def add_task(title: str, category: str, priority: str, goal_id: Optional[int] = None, est_time: float = 1.0):
    with Session(engine) as session:
        new_task = Task(title=title, category=category, priority=priority, goal_id=goal_id, estimated_time=est_time)
